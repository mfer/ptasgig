...
int main(int argc, char **argv)
{
    ...
    if(!gtk_builder_add_from_file(builder, "gui/mwis.glade", &error)) {
        if(error) {
    ...
    gtk_file_filter_set_name(file_filter, "Custom data file (*.mwis)");
    gtk_file_filter_add_pattern(file_filter, "*.mwis");
...
void process(GtkButton* button, gpointer user_data)
{
    ...
    if(!report_created) {
        ...
        int as_pdf = latex2pdf("mwis", "reports");
        if(as_pdf == 0) {
...
void save_cb(GtkButton* button, gpointer user_data)
{
    ...
    /* Check extension */
    if(!g_str_has_suffix(filename, ".mwis")) {
        char* new_filename = g_strdup_printf("%s.mwis", filename);
        g_free(filename);
